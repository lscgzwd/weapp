...
class WxPay
{
    ...
    /**
     * 被扫支付，即刷卡支付
     * @param string $auth_code 扫码串
     * @param string $out_trade_no 订单号
     * @param string $body 商品描述
     * @param number $amount 金额 单位为 分
     * @return bool|array
     */
     ...
     * @param string $accessToken oauth后获取的令牌
     * @return string
     */
     ...
     * @param string $billType 账单类型 ALL，返回当日所有订单信息，默认值 SUCCESS，返回当日成功支付的订单  REFUND，返回当日退款订单
     * @param string $tarType 压缩账单 非必传参数，固定值：GZIP，返回格式为.gzip的压缩包账单。不传则默认为数据流形式。
     * @return bool|array
     */
     ...
     * @param string $refundNo  退款号
     * @param string $transactionIdOrOutTradeNo 微信的订单号或者商户订单号
     * @param int $type 单号类型 0 微信的订单号,1 商户订单号
     * @return bool|array
     */
    ...
    /**
     * 退款查询
     * @param string $refundQueryKey 退款查询标示，可以是退款单号refund_id、out_refund_no、out_trade_no、transaction_id其中一个。
     ...
     * </xml>
     */
    public static function payNotify($callBackFunc, $xml = null)
    {
        $WxPayNotifyCb = new WxPayNotifyCb($callBackFunc);
        $WxPayNotifyCb->Handle(false, $xml);
    }

    /**
     * 检查支付回调数据签名是否正确
     * @param array $cfg 支付配置是否正确
     * @param $data
     * @return bool
     */
